...
# Defining function to check if user has proper permissions
...
def check_permissions(permission, payload):
    ...
    # Checking to see if permission from JWT matches
    # what's available in general
...
# Defining a function to check that the provided token
# matches what is expected from Auth0
def verify_decode_jwt(token):
    ...
    # Appending information to RSA key dictionary from jwks if 'kid'
    # matches the unverified header
    ...
    # Getting payload information from the token using key if
    # everything checks out fine (else raises error)
    if rsa_key:
        try:
            payload = jwt.decode(
                token,
...
# Defining a function to create a nice Python decorator to
# easily ensure if auhtoirzation info is present
